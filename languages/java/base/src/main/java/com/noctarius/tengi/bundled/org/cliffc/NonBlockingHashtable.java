...
package com.noctarius.tengi.bundled.org.cliffc;
    ...
    implements ConcurrentMap<TypeK, TypeV>, Cloneable, Serializable
{
                                          ...
                                          final int fullhash )
    {
        ...
        while ( true )
        {
                ...
                key == TOMBSTONE ) // found a TOMBSTONE key, means no more keys in this table
            {
                return newkvs == null
                    ? null
                    : get_impl( topmap, topmap.help_copy( newkvs ), key, fullhash ); // Retry in the new table
            }
    ...
    private static final class CHM<TypeK, TypeV>
    {
        ...
        // read it yet.
        private final Object[] copy_slot_and_check( NonBlockingHashtable topmap, Object[] oldkvs, int idx,
                                                    Object should_help )
        {
            assert chm( oldkvs ) == this;
            Object[] newkvs = _newkvs; // VOLATILE READ
